...
def compute_metrics(p, id2label, valid_ds, valid_df, threshold=0.9, is_valid=False):
    """
    Compute the LB metric (lb) and other auxiliary metrics
    """
    predictions, _ = p

    pred_df = parse_predictions(predictions, id2label, valid_ds, threshold)
    ...
    return (threshold, final_results) if is_valid else final_results


def evaluate(pred_df, valid_df):
    """
    Compute the LB metric (lb) and other auxiliary metrics
    """
    references, predictions = set(get_triplets(valid_df)), get_triplets(pred_df)
...
def get_f5_at_different_thresholds(preds, id2label, valid_ds, reference_df):
